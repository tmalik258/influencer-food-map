...
async def download_audio(video_url: str, video: Video) -> str:
    ...
    influencer_name = video.influencer.name.replace(" ", "_").replace("/", "_") if video.influencer else "unknown"
    influencer_dir = os.path.join(AUDIO_BASE_DIR, influencer_name)
    os.makedirs(influencer_dir, exist_ok=True)
    
    # Sanitize video title for filename
    video_title = video.title.replace(" ", "_").replace("/", "_").replace("\\", "_")[:50] if video.title else str(uuid.uuid4())
    ...
    for attempt in range(max_retries):
        ...
        temp_file = None
        try:
            logger.info(f"Attempt {attempt + 1}/{max_retries} downloading audio for {video_url}")
            ...
            logger.error(f"Attempt {attempt + 1}/{max_retries} failed for {video_url}: {e}")
            # Clean up files on error
            if downloaded_file and os.path.exists(downloaded_file):
                os.remove(downloaded_file)
            if os.path.exists(final_output_path):
            ...
            logger.error(f"Unexpected error on attempt {attempt + 1}/{max_retries} for {video_url}: {e}")
            # Clean up files on error
            if downloaded_file and os.path.exists(downloaded_file):
                os.remove(downloaded_file)
            if os.path.exists(final_output_path):
...
async def extract_entities(transcription: str) -> dict:
    """Extract restaurant name, location, context, and tags using GPT-4 with referer header."""
    loop = asyncio.get_event_loop()
    try:
        prompt = f"""
        ...
        """
        logger.info(f"Extracting entities from prompt: {prompt}")
...
async def validate_restaurant(entities: dict) -> dict:
    ...
    try:
        ...
        if result["status"] == "OK" and result["results"]:
            ...
            logger.info(f"Validated restaurant with Google Maps: {json.dumps(place, indent=2)}")
            return {
                "valid": True,
                "name": place["name"],
...
async def process_video(db: AsyncSession, video: Video):
    ...
    try:
        ...
        if not video.transcription:
            logger.info(f"No transcription found for video {video.youtube_video_id}, downloading and transcribing...")
            # Download and transcribe
            audio_path = await download_audio(video.video_url, video)
            transcription = await transcribe_video(audio_path)

            logger.info(f"Transcription completed for video {video.youtube_video_id}: {transcription[:255]}...")
        ...
        if not entities:
            ...
            return
        logger.info(f"Entities extracted for video {video.youtube_video_id}: {entities}")
        ...
        if not validated["valid"]:
            ...
            return
        logger.info(f"Validated restaurant for video {video.youtube_video_id}: {validated}")
        
        # Store restaurant, tags, and listing
        await store_restaurant_and_listing(db, video, entities, validated)

        logger.info(f"Stored restaurant, tags, and listing for video {video.youtube_video_id}")
    except Exception as e:
