...
interface InfluencersHeroProps {
  loading: boolean;
  influencersWithStats: InfluencerWithStats[];
}

export default function InfluencersHero({ loading, influencersWithStats }: InfluencersHeroProps) {
  return (
            ...
            <div className="flex flex-wrap justify-center items-center gap-8 mb-8">
              ...
              <div className="flex items-center gap-3 bg-white/10 backdrop-blur-sm px-6 py-3 rounded-full border border-white/20">
                ...
                <span className="text-white font-semibold">
                  {new Set(influencersWithStats.map(i => i.region).filter(Boolean)).size} Regions
                </span>
              </div>
              <div className="flex items-center gap-3 bg-white/10 backdrop-blur-sm px-6 py-3 rounded-full border border-white/20">
                <Award className="w-5 h-5 text-orange-400" />
                <span className="text-white font-semibold">
                  {influencersWithStats.reduce((sum, inf) => sum + inf.totalVideos, 0)} Reviews
                </span>
              </div>
            </div>
          </>
