...
const formSchema = z.object({
  ...
  youtube_url: z.string().url("Please enter a valid YouTube URL").refine(
    (url) => {
...
export function VideoCreateForm({ onSuccess }: VideoCreateFormProps) {
  ...
  const { influencers, loading: isLoadingInfluencers, error: influencersError } = useInfluencers();
  const { createVideoFromUrl, loading: isCreating, error: createError } = useCreateVideo();
  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <Select onValueChange={field.onChange} value={field.value} disabled={isLoadingInfluencers}>
                <FormControl>
                  <SelectTrigger className="w-full glass-effect backdrop-blur-sm bg-white/50 dark:bg-gray-800/50 border-orange-200 dark:border-orange-800 focus:border-orange-500 focus:ring-orange-500/20 text-gray-900 dark:text-white">
                    <SelectValue placeholder={isLoadingInfluencers ? "Loading influencers..." : "Select an influencer"} />
                  </SelectTrigger>
                </FormControl>
                <SelectContent>
        ...
        />
        <Button type="submit" disabled={isCreating || isLoadingInfluencers} className="bg-orange-600 hover:bg-orange-700 text-white border-orange-600 hover:border-orange-700 transition-all duration-200 disabled:cursor-not-allowed cursor-pointer">
